...
class MenuContentPageState extends State<MenuContentPage> {
  ...
  final List<GlobalKey> _ingredientKeys = [];
  final List<GlobalKey> _procedureKeys = [];
  ...
  // ✅ New: Completed procedures tracking
  final Set<int> _completedProcedures = {};
  ...
  // ✅ New: Toggle procedure completion
  void _toggleProcedureCompletion(int index) {
    setState(() {
      if (_completedProcedures.contains(index)) {
        _completedProcedures.remove(index);
      } else {
        _completedProcedures.add(index);
      }
    });
  }
  ...
  void initState() {
    ...
    // ✅ Initialize keys for auto-scroll
    _ingredientKeys.addAll(
      List.generate(widget.ingredients.length, (_) => GlobalKey()),
    );
    _procedureKeys.addAll(
      List.generate(widget.procedures.length, (_) => GlobalKey()),
    );
  ...
  Future<void> _askUntilAnswered({
    ...
    // Only speak if question is not empty
    ...
    await _speech.listen(
      ...
      onResult: (res) async {
        if (res.finalResult && res.recognizedWords.isNotEmpty) {
          ...
          // Repeat current step when no valid response
          Future.delayed(const Duration(milliseconds: 1000), () {
            if (mounted && !_waitingForResponse && _stateStep == 2) {
              _readNextIngredient(); // Repeat same ingredient
            } else if (mounted && !_waitingForResponse && _stateStep == 4) {
              _readNextProcedure(); // Repeat same procedure
            } else if (mounted && !_waitingForResponse) {
              _askUntilAnswered(
                question: question,
    ...
    if (_waitingForResponse && questionId == _currentQuestionId) {
      ...
      // Repeat current step on timeout
      Future.delayed(const Duration(milliseconds: 1000), () {
        if (mounted && !_waitingForResponse && _stateStep == 2) {
          _readNextIngredient(); // Repeat same ingredient
        } else if (mounted && !_waitingForResponse && _stateStep == 4) {
  ...
  void _readNextIngredient() async {
    ...
    // ✅ Auto-scroll to current ingredient
    _scrollToIngredient(_ingredientIndex);
    ...
    _askUntilAnswered(
      question: "", // Empty question since we already spoke
  ...
  void _readNextProcedure() async {
    final list = _getProcedures();

    if (_procedureIndex < list.length) {
      _stepCompleted = false;

      // ✅ Auto-scroll to current procedure
      _scrollToProcedure(_procedureIndex);
      ...
      void handleCommand(String cmd) {
        ...
        if (hasBackKeyword) {
          ...
          if (num != null && num > 0 && num <= list.length) {
            // Remove highlights for steps after the target step
            setState(() {
              _completedProcedures.removeWhere((step) => step >= num - 1);
            });
            _procedureIndex = num - 1;
            _stepCompleted = true;
            _readNextProcedure();
            return;
          }
        ...
        setState(() {
          _completedProcedures.add(_procedureIndex);
        });
  ...
  List<String> _getIngredients() {
    return _ingredientData.asMap().entries.map((entry) {
      ...
      // Use translated name for display if in Tagalog
      final displayName =
          _language == 'fil'
              ? _getTranslatedIngredientName(substitute)
              : substitute;
  ...
  void _scrollToIngredient(int index) {
    if (index < _ingredientKeys.length &&
        _ingredientKeys[index].currentContext != null) {
      final RenderBox? renderBox =
          _ingredientKeys[index].currentContext!.findRenderObject()
              as RenderBox?;
      if (renderBox != null) {
        final position = renderBox.localToGlobal(Offset.zero);
        final screenHeight = MediaQuery.of(context).size.height;

        // Only scroll if item is not visible on screen
        if (position.dy < 0 || position.dy > screenHeight - 100) {
          Scrollable.ensureVisible(
            _ingredientKeys[index].currentContext!,
  ...
  void _scrollToProcedure(int index) {
    if (index < _procedureKeys.length &&
        _procedureKeys[index].currentContext != null) {
      final RenderBox? renderBox =
          _procedureKeys[index].currentContext!.findRenderObject()
              as RenderBox?;
      if (renderBox != null) {
        final position = renderBox.localToGlobal(Offset.zero);
        final screenHeight = MediaQuery.of(context).size.height;
        ...
        if (position.dy < 0 || position.dy > screenHeight - 100) {
          Scrollable.ensureVisible(
            _procedureKeys[index].currentContext!,
            duration: const Duration(milliseconds: 300),
            curve: Curves.easeInOut,
          );
        }
              ...
              children: [
                ...
                ...proceduresList.asMap().entries.map(
                  (entry) => GestureDetector(
                    onTap: () => _toggleProcedureCompletion(entry.key),
                    child: Padding(
                      key: _procedureKeys[entry.key],
                      padding: const EdgeInsets.symmetric(
                        horizontal: 24,
                        vertical: 8,
                      ),
                      child: Container(
                        decoration: BoxDecoration(
                          color: _completedProcedures.contains(entry.key)
                              ? Colors.green.withValues(alpha: 0.1)
                        ...
                        child: Padding(
                          ...
                          child: Row(
                            ...
                            children: [
                                ...
                                child: Text(
                                  ...
                                  style: TextStyle(
                                    ...
                                    color: _completedProcedures.contains(entry.key)
                                        ? Colors.green[700]
                                        : Colors.black87,
                                    fontWeight: _completedProcedures.contains(entry.key)
                                        ? FontWeight.w600
                                        : FontWeight.normal,
                                  ),
                                ),
                              ...
                              // ✅ Completed indicator
                              if (_completedProcedures.contains(entry.key))
                                Container(
                                  padding: const EdgeInsets.symmetric(
                                    horizontal: 8,
                                    vertical: 4,
